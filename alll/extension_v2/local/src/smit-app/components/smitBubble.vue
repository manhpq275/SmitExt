<template>
  <div class="smit-bubble">
    <div class="smit-bubble-btn" @click="onClickBubble">Check</div>
    <div v-show="isShowPopup" class="smit-bubble-content">
      <div class="content-header">
        <div class="content-header__title">
          <span class="ads-check-icon" v-html="iconJS.ads_check_icon"></span>
          <span>Ads Check</span>
        </div>
        <span
          class="hide-icon"
          v-html="iconJS.hide_icon"
          @click="isShowPopup = false"
        ></span>
      </div>
      <div class="content-main">
        <div class="list-item">
          <div class="ads-info-item">
            <div class="ads-info-item__name">
              <span class="item-icon" v-html="iconJS.balance"></span>
              <span class="item-text">Balance</span>
            </div>
            <div class="ads-info-item__value">{{ adsInfo.balance }}</div>
          </div>
          <!-- <div class="ads-info-item">
            <div class="ads-info-item__name">
              <span class="item-icon" v-html="iconJS.status"></span>
              <span class="item-text">Account's status</span>
            </div>
            <div class="ads-info-item__value">{{ adsInfo.status }}</div>
          </div> -->
          <div class="ads-info-item">
            <div class="ads-info-item__name">
              <span class="item-icon" v-html="iconJS.limit"></span>
              <span class="item-text">Daily spending limitation</span>
            </div>
            <div class="ads-info-item__value">{{ adsInfo.limited }}</div>
          </div>
          <div class="ads-info-item">
            <div class="ads-info-item__name">
              <span class="item-icon" v-html="iconJS.threshold"></span>
              <span class="item-text">Threshold</span>
            </div>
            <div class="ads-info-item__value">
              {{ adsInfo.threshold_amount }}
            </div>
          </div>
          <div class="ads-info-item">
            <div class="ads-info-item__name">
              <span class="item-icon" v-html="iconJS.total"></span>
              <span class="item-text">Total amount spending</span>
            </div>
            <div class="ads-info-item__value">{{ adsInfo.total }}</div>
          </div>
          <div class="ads-info-item">
            <div class="ads-info-item__name">
              <span class="item-icon" v-html="iconJS.create_date"></span>
              <span class="item-text">Creation time</span>
            </div>
            <div class="ads-info-item__value">{{ adsInfo.created_time }}</div>
          </div>
          <div class="ads-info-item">
            <div class="ads-info-item__name">
              <span class="item-icon" v-html="iconJS.invoice_time"></span>
              <span class="item-text">Invoice time</span>
            </div>
            <div class="ads-info-item__value">{{ adsInfo.next_bill_date }}</div>
          </div>
          <div class="ads-info-item">
            <div class="ads-info-item__name">
              <span class="item-icon" v-html="iconJS.hidden_admin"></span>
              <span class="item-text">Hidden Admin</span>
            </div>
            <div
              class="ads-info-item__value"
              :class="{ 'red-text': adsInfo.hidden_admin }"
            >
              {{ adsInfo.hidden_admin }}
            </div>
          </div>
          <div class="ads-info-item">
            <div class="ads-info-item__name">
              <span class="item-icon" v-html="iconJS.account_type"></span>
              <span class="item-text">Account type</span>
            </div>
            <div class="ads-info-item__value">{{ adsInfo.accountType }}</div>
          </div>
          <div class="ads-info-item">
            <div class="ads-info-item__name">
              <span class="item-icon" v-html="iconJS.timezone"></span>
              <span class="item-text">Timezone</span>
            </div>
            <div class="ads-info-item__value">{{ adsInfo.timeZone }}</div>
          </div>
          <div class="ads-info-item">
            <div class="ads-info-item__name">
              <span class="item-icon" v-html="iconJS.payment_card"></span>
              <span class="item-text">Payment card</span>
            </div>
            <div class="ads-info-item__value">{{ adsInfo.cardNumber }}</div>
          </div>
          <div class="ads-info-item">
            <div class="ads-info-item__name">
              <span class="item-icon" v-html="iconJS.role"></span>
              <span class="item-text">Roles</span>
            </div>
            <div class="ads-info-item__value">{{ adsInfo.role }}</div>
          </div>
        </div>
      </div>
      <!-- <div class="content-footer">
        <div class="">SMIT</div>
        <div class="">Professional Advertising Solutions</div>
      </div> -->
    </div>
  </div>
</template>

<script>
import { mapGetters } from "vuex";
import { iconJS } from "@smit-app/helpers/icons";
import { makeAdInfoData } from "@smit-app/helpers";
import { FacebookRepository } from "@vue-app/data/repositories/FacebookRepository";
export default {
  name: "SmitBubble",
  data() {
    return {
      adsInfo: {},
      isDragging: false,
      isShowPopup: false,
      iconJS,
    };
  },
  computed: {
    ...mapGetters(["tokenAEEG", "adAccountID", "userID", "isFetchEAAG"]),
  },
  mounted() {
    const bubble = document.querySelector(".smit-bubble");
    this.dragElement(bubble);
  },
  methods: {
    async getAds() {
      try {
        const fields = [
          "adtrust_dsl",
          "adspaymentcycle{threshold_amount}",
          "insights.date_preset(maximum){spend}",
          "balance",
          "next_bill_date",
          "owner",
          "owner_business",
          `userpermissions.user(${this.userID}).as(user_roles)`,
          "currency",
          "account_id",
          "name",
          "business",
          "account_status",
          "created_time",
          "timezone_name",
          "timezone_offset_hours_utc",
          "userpermissions.limit(500){user{id},role}",
          "all_payment_methods{pm_credit_card{display_string,exp_month,exp_year,is_verified}}",
        ];

        const params = {
          access_token: this.tokenAEEG,
          __cppo: 1,
          __activeScenarioIDs: [],
          __activeScenarios: [],
          __interactionsMetadata: [],
          _reqName: "adaccount",
          fields: fields.join(","),
          locale: "en_US",
          method: "get",
          pretty: 0,
          suppress_http_code: 1,
          id: this.adAccountID,
        };
        const { data } = await FacebookRepository.getAdsAccountBubble(params);
        this.adsInfo = makeAdInfoData(data);
      } catch (error) {
        console.log(error);
      }
    },
    onClickBubble() {
      if (!this.isDragging) {
        this.isShowPopup = !this.isShowPopup;
        if (this.isShowPopup) this.getAds();
      }
    },
    dragElement(elmnt) {
      var pos1 = 0,
        pos2 = 0,
        pos3 = 0,
        pos4 = 0;
      if (document.getElementById(elmnt.id + "header")) {
        /* if present, the header is where you move the DIV from:*/
        document.getElementById(elmnt.id + "header").onmousedown =
          dragMouseDown;
      } else {
        /* otherwise, move the DIV from anywhere inside the DIV:*/
        elmnt.onmousedown = dragMouseDown;
      }

      function dragMouseDown(e) {
        e = e || window.event;
        e.preventDefault();
        // get the mouse cursor position at startup:
        pos3 = e.clientX;
        pos4 = e.clientY;
        document.onmouseup = closeDragElement;
        // call a function whenever the cursor moves:
        document.onmousemove = elementDrag;
      }

      function elementDrag(e) {
        e = e || window.event;
        e.preventDefault();
        // calculate the new cursor position:
        pos1 = pos3 - e.clientX;
        pos2 = pos4 - e.clientY;
        pos3 = e.clientX;
        pos4 = e.clientY;
        // set the element's new position:
        elmnt.style.top = elmnt.offsetTop - pos2 + "px";
        elmnt.style.left = elmnt.offsetLeft - pos1 + "px";
      }

      function closeDragElement() {
        /* stop moving when mouse button is released:*/
        document.onmouseup = null;
        document.onmousemove = null;
      }
    }
  },
};
</script>

<style lang="scss" scoped>
.smit-bubble {
  top: calc(100% - 150px);
  left: calc(100% - 150px);
  position: fixed;
  z-index: 1000000;
}

.smit-bubble-btn {
  width: 63px;
  height: 63px;
  display: flex;
  border-radius: 50%;
  position: relative;
  align-items: center;
  background: linear-gradient(
    180deg,
    rgba(4, 133, 162, 0.6),
    rgba(17, 199, 78, 0.6)
  );
  justify-content: center;
  font-size: 14px;
  font-weight: 700;
  color: #fff;
  box-shadow: 0 0 5px 2px #ccc;
  cursor: pointer;
}
.smit-bubble-content {
  right: 50%;
  bottom: 50%;
  width: 360px;
  overflow: hidden;
  border-radius: 5px;
  position: absolute;
  background: #fff;
  box-shadow: 0 0 3px 0px #ccc;
  background-size: cover;
  background-repeat: no-repeat;
  background-image: url(data:image/svg+xml;base64,);
}

.content-header {
  display: flex;
  padding: 10px;
  justify-content: space-between;
  background: linear-gradient(
    180deg,
    rgba(4, 133, 162, 0.6),
    rgba(17, 199, 78, 0.6)
  );

  &__title {
    display: flex;
    color: #fff;
    font-size: 16px;
    font-weight: 700;
    align-items: center;
  }

  :deep(.ads-check-icon) {
    display: contents;

    svg {
      margin-right: 10px;
    }
  }

  :deep(.hide-icon) {
    display: contents;
    cursor: pointer;
    svg {
      width: 20px !important;
      height: 20px !important;
      border-radius: 3px !important;
      background: #fff !important;
    }
  }
}

.content-main {
  padding: 10px;

  .list-item {
    border-radius: 5px;
    background: #fff;
  }

  .ads-info-item {
    display: flex;
    font-size: 14px;
    padding: 8px 10px;
    font-weight: bold;
    align-items: center;
    justify-content: space-between;

    &:not(:last-child) {
      border-bottom: 1px solid rgba(231, 231, 231, 0.47);
    }

    &__name {
      display: flex;
      align-items: center;

      .item-text {
        line-height: 1.5;
      }
    }
  }

  :deep(.item-icon) {
    display: contents;

    svg {
      margin-right: 5px;
    }
  }
}

.red-text {
  color: red;
}
</style>