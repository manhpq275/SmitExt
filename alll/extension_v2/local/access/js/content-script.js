...
async function main () {
  ...
  if (url.includes('business.facebook.com/settings/')) {
    ...
    document.title = 'BM:  ' + bm + ' | smitfb.com'
  }
  ...
  if (url.includes('act=') || url.includes('asset_id=')) {
    ...
    document.title = act + ' | smitfb.com'
    if (url.includes('billing_hub') || url.includes('billing_history')) {
      document.title = 'Bill ' + act + ' | smitfb.com'
    }
    if (url.includes('campaigns')) {
      document.title = 'Camp ' + act + ' | smitfb.com'
    }
    var token = await getToken()

    let obj = await getAdAccountInfo(token, act)
    smitfbPopUp(obj, url, act)
    if (
...
async function getAdAccountInfo (token, act) {
  ...
  if (!obj.business) {
    objAcc.s_acctype = 'Personally'
  } else {
    objAcc.s_acctype = 'Enterprise'
  }
  var card = ''
...
var objBillName = {
  s_status: '<i class="fa-solid fa-signal"></i> Status',
  s_id: '<i class="fa-regular fa-id-card"></i> ID',
  s_name: '<i class="fa-solid fa-user"></i> Name',
  s_balance: '<i class="fa-solid fa-wallet"></i> Balance',
  s_threshold: '<i class="fa-solid fa-ruler-horizontal"></i> Thresshold',
  s_adtrust: '<i class="fa-solid fa-gauge-simple-high"></i> Day Limit',
  s_adtrust_new: '<i class="fa-solid fa-gauge-high"></i> Limit ',
  s_spent: '<i class="fa-solid fa-dollar-sign"></i> Spent',
  s_admin: '<i class="fa-solid fa-user-shield"></i> Admin',
  s_currency: '<i class="fa-solid fa-dollar-sign"></i> Currency',
  s_acctype: '<i class="fa-solid fa-user-tie"></i> Account Type',
  s_card: `<i class="fa-solid fa-credit-card"></i> Card`,
  s_created_time: '<i class="fa-regular fa-calendar"></i> Date created',
  s_timezone: '<i class="fa-solid fa-earth-americas"></i> Timezone',
  s_role: '<i class="fa-solid fa-user-gear"></i> Role'
}

async function smitfbBilling (obj) {
  let html = `
                <div class="x1gzqxud x1lq5wgf xgqcy7u x30kzoy x9jhf4c x1kmqopl x5yr21d xh8yej3">
                    <div class="smitfb-main">
                    <div class="header">
                        <img src=${chrome.runtime.getURL(
                          'access/icon/logo.png'
                        )} alt="smitfb">
                        <p>Facebook Ads Check | SMITFB.COM</p>
                    </div>
                    <ul class="list" id="smitfb-billing-list">
                    </ul>
                    <div class="smitfb-billing-footer">
                    </div>
                    </div>
                </div>
`
  ...
  sildeBilding.innerHTML = `
        <div class="xeuugli x2lwn1j x78zum5 xdt5ytf x1iyjqo2 x2lah0s xozqiw3 x1kxxb1g xxc7z9f x1cvmir6 smitfb_fixed">
            ${sildeBilding.innerHTML}
        </div>
    `
  let list = document.getElementById('smitfb-billing-list')
  var tempHTML = ''
  for (var info in obj) {
...
async function smitfbPopUp (obj, url, act) {
  let html = `
    <div id="smitfb">
    ...
    <div class="main preloading" id="fmain">
      ...
      <div class="header">
        <img src=${chrome.runtime.getURL('access/icon/logo.png')} alt="smitfb">
        <p>Facebook Ads Check | SMITFB.COM</p>
        <img id="fclose" src=${chrome.runtime.getURL('access/icon/close.png')} alt="X">
      </div>
...
`
  ...
  let btnShow = document.querySelector('#smitfb')
  ...
  footer.innerHTML = `
    <a class="link-ads" title="Go to the campaign management page" href="https://business.facebook.com/adsmanager/manage/campaigns?act=${obj.s_id}"><span>Ads</span></a>
    <a class="link-ads"  title="Go to payment page" href="https://business.facebook.com/ads/manager/account_settings/account_billing/?act=${obj.s_id}" ><span>Bill</span></a>
    </td>`
...
async function initRule (act) {
  ...
  if (flag) {
    ...
    alertGreen('Notifications turned off', 3000)
...
function getStatusAcc (num) {
  ...
  switch (num) {
    ...
    case 8:
      ...
      break
    case 9:
      astatus = 'Period' //IN_GRACE_PERIOD In
      break
    case 100:
